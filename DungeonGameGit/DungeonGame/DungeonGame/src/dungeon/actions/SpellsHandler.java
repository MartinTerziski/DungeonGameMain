...
public class SpellsHandler {
	...
	public static boolean useSpell(Spells spells, Role role, BasicMonster basicMonster, String input) {
		...
		if(role instanceof Juggernaut) {
			switch(input) {
			case "1":
				usedSpell = JuggernautSpells.resolveEmpowerAttack(5, spells, role, basicMonster, input);
				break;
			case "2":
				usedSpell = JuggernautSpells.resolveImmunizingShield(10, spells, role, basicMonster, input);
				break;
			default:
		...
		if(role instanceof SpellInvoker) {
			switch(input) {
			case "1":
				usedSpell = SpellInvokerSpells.resolveStormBolt(5, spells, role, basicMonster, input);
				break;
			case "2":
				usedSpell = SpellInvokerSpells.resolveFireComet(20, spells, role, basicMonster, input);
				break;
			case "3":
				usedSpell = SpellInvokerSpells.resolveIceShard(10, spells, role, basicMonster, input);
				break;
			default:
				System.out.println("Invalid Command!");
			}
		...
		if(role instanceof Marksman) {
			switch(input) {
			case "1":
				usedSpell = MarksmanSpells.resolvePoisonArrow(10, spells, role, basicMonster, input);
				break;
			case "2":
				usedSpell = MarksmanSpells.resolveFollowupArrows(5, spells, role, basicMonster, input);
				break;
			case "3":
				usedSpell = MarksmanSpells.resolveCriticalArrow(5, spells, role, basicMonster, input);
				break;
			default:
				System.out.println("Invalid Command!");
			}
